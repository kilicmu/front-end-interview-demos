...
// dynamic create entry
const basePagesPath = resolve(__dirname, 'src', 'pages')
const pagesDirs = fs.readdirSync(basePagesPath);
console.log(pagesDirs)
const entry = pagesDirs.reduce((entry, dir) => ({ ...entry, [dir]: join(basePagesPath, dir, 'index.js') }), { vendor: ['lodash'] })
const htmlWebpackPlugins = pagesDirs.reduce((collection, dir) => {
    collection.push(new HtmlWebpackPlugin({ template: join(basePagesPath, dir, 'index.html'), chunks: [dir], filename: `${dir === "page1" ? "index" : dir}.html` }));
    return collection;
}, [])
    ...
    return {
        ...
        */
        ...
        output: {
            ...
            filename: '[name].[chunkhash:10].js',// 入口模块
            // 代码块name来源：
            // 非入口代码块有两个来源：
            /**
             ...
             * 2. 懒加载 import()
             */
            chunkFilename: '[name].[chunkhash:5].js', // 非入口模块
